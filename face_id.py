...
# a function that is similar to the encoding process above but used in the while loop
def encoder():
    face_encodings = []
    # picture_count = 0
    for file in os.listdir('photos/'):
...
# a function that is used in the while loop to update the list of users
def check():
    #check_dict = {}
    face_names = []
    files = os.listdir('photos/')
...
while True:
    check_dict = {}
    if len(check()) != len(known_face_names):
        encoder()
        known_face_encodings = encoder()
        known_face_names = check()
        for item in check():
            check_dict[item] = False
        user_dict = check_dict
        # print(known_face_names)
        # print(known_face_encodings)
...
# in case of no faces recognized set all users to False
    ...
    if process_this_frame:
        ...
        for face_encoding in face_encodings:
            ...
            if matches[best_match_index]:                                  # important#
